...
def setup_tomcat(java_home='/usr/java/', version='v9.0.14'):
    def func(session):
        ...
        if not shell.check_file_exists(session, tgz):
            url = 'http://archive.apache.org/dist/tomcat/tomcat-9/%s/bin/%s' % (version, tgz)
            download = ' '.join([
                'wget',
                '-4',
                url
...
def setup(session):
    tomcat_path = setup_tomcat()(session)
